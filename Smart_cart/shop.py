...
def Map():
    ...
    global Mystate, windowSurface, goal, my_pos, previous_right_time, previous_state_time
    # some definition
    ...
    while True:
        ...
        # build surface
        windowSurface.blit(background, (0, 0))
        windowSurface.blit(home_icon, (200, 505))
        if item_now is not None:
            windowSurface.blit(info_icon, (300, 500))
            windowSurface.blit(cab_icon,  (400, 503))
    ...
    while True:
        ...
        if GPIO.input(ENTER) == 0:
            ...
            if Mystate == Info_state['HOME']:
                ...
                if item_list[item_now[7]][2] == 0:
                    item_list[item_now[7]] = [item_now[0], item_now[1], 1]
                else:
                    item_list[item_now[7]][2]+=1
                total += item_now[1]
                Car()
        elif GPIO.input(SWITCH) == 0:
            if (time.time()-previous_right_time) > 0.2:
        ...
        number = font.render("bargains in last week: " + str(item_now[3]), True, (0,0,225))
    
        # build surface
        windowSurface.blit(background, (0, 0))
        windowSurface.blit(home_icon, (200, 505))
        windowSurface.blit(map_icon, (300, 500))
        windowSurface.blit(scan_icon,  (400, 503))
...
def Car():
    ...
    global Mystate, windowSurface, total, item_now, item_list, previous_state_time
    ...
    while True:
        ...
        # build surface
        windowSurface.blit(background, (0, 0))
        windowSurface.blit(home_icon, (300, 510))
        windowSurface.blit(text,  (600, 500))
        if total != 0:
            string = 'Name:    number:    price:'
            str1 = font.render(string, True, (50, 50, 50))
            str2 = font.render(str(total), True, (200, 0, 0))
            windowSurface.blit(str1, (200, 150))
            windowSurface.blit(str2, (700, 75))
        count = 0
        for i in range(21):
            if item_list[i][2] != 0:
                count+=1
...
def Cab():
    #cab page: 
    global Mystate, windowSurface, previous_state_time
